...
function FadeBlock({ children, active, delay = 0 }: { children: React.ReactNode; active: boolean; delay?: number }) {
  return (
    <div
      className={`transition-all duration-700 ease-out will-change-transform ${active ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-3'}`}
...
export default function Services({ selectedVehicleSize }: ServicesProps) {
  ...
  const previousSize = useRef(selectedVehicleSize);

  const size = selectedVehicleSize || 'pequeno';
  const vehicleLabel = sizeLabelMap[size] ?? size;

  useEffect(() => {
    if (previousSize.current !== selectedVehicleSize) {
      setAnimateKey((value) => value + 1);
      previousSize.current = selectedVehicleSize;
    }
  }, [size]);
  ...
  return (
    <section id="services" className="bg-black px-4 sm:px-6 pb-16 md:pb-24">
      <div className="max-w-7xl mx-auto space-y-16 md:space-y-24">
        <FadeBlock key={`label-${animateKey}`} active={true} delay={0}>
        ...
        <FadeBlock key={`premium-${animateKey}`} active={true} delay={90}>
        ...
        <div key={`basic-${animateKey}`} className="transition-all duration-700 ease-out animate-[fadeUp_.7s_ease-out]">
